...
def load_data(base_path):
    x_train = np.load('{}dataswedish_leaf64x64pix_train_images.npy'.format(base_path))
    x_train = (x_train.astype(np.float32) - 127.5) / 127.5
    y_train = np.load('{}dataswedish_leaf64x64pix_train_labels.npy'.format(base_path)) - 1
...
#     y_train = np.concatenate((y_train, np.load('{}swedish_leaf_generator64_50000_2_labels.npy'.format(base_path))))
    ...
    x_test = np.load('{}dataswedish_leaf64x64pix_test_images.npy'.format(base_path))
    x_test = (x_test.astype(np.float32) - 127.5) / 127.5
    y_test = np.load('{}dataswedish_leaf64x64pix_test_labels.npy'.format(base_path)) - 1

    return (x_train, y_train), (x_test, y_test)


class AlexNet:
    def __init__(self, x_train, y_train, x_test, y_test, lr=0.00001, folder='', run_nr='', num_classes=10):
        """
    ...
    def build_network(self):
        ...
        # Output Layer
        alexnet.add(Dense(self.num_classes))
...
if __name__ == '__main__':
    ...
    epochs = 6
    lr = 0.0001 # 0.00001 best till now
    from pipeline import split_data
    x_train, y_train, x_test, y_test = split_data(split=0.5)

    alexnet = AlexNet(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, lr=lr, folder='test/')

    alexnet.train_network_with_generator(epochs=epochs, save_model=False, save_csv=False)
