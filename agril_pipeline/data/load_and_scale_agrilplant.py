...
def load_one_class(class_path, size, split):
    ...
    labels = [int(filename.split('_')[1].split(".")[0]) for filename in image_filenames]  # Needs to be changed!
...
def load_agrilplant_in_right_format(path, size, split):
    ...
    for idx, class_name in enumerate(classes):
        print("({}/{}) Loading {} images...".format(idx + 1, len(classes), class_name))
        (x_train_temp, y_train_temp, x_test_temp, y_test_temp) = load_one_class(os.path.join(path, class_name), size, split)
        x_train.extend(x_train_temp)
        y_train.extend([idx+1]*len(x_train_temp))
        x_test.extend(x_test_temp)
        y_test.extend([idx+1]*len(x_test_temp))
...
(x_train, y_train, x_test, y_test) = load_agrilplant_in_right_format(DATADIR, size, split)
...
np.save('{}agrilplant{}x{}pix_all_images'.format(NPY_STORAGE, size[0], size[1]), x_train)
np.save('{}agrilplant{}x{}pix_all_labels'.format(NPY_STORAGE, size[0], size[1]), y_train)
